          ...
          LandingsGallery.map((item, index) => {
            return (
              <div key={index} className="py-2 w-full">
                <h3 className="text-4xl text-secondary font-bold text-center pb-10">{item.nameLanding}</h3>
                <Masonry
                    ...
                    item.imgLanding.map((img, index) => (
                      <div key={index} onClick={() => openModal(index, item.imgLanding)}>
                        <LazyImage
                          src={img}
                          alt={`gallery-${index}`}
                          className="w-full h-auto object-cover cursor-pointer transition duration-300 transform hover:scale-105 hover:shadow-lg hover:border-red-700 hover:border-4"
                        />
                      </div>
                    ))
                  }
